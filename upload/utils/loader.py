...
def load_sentences(path, lower, zeros):
    ...
    input:
        path: path string
        lower: whether all vocabularies need lower case
        zeros: whether covert all number digit to '0'

    return:
        list(list): [[word, part-of-speech tag, chunk tag, named entity tag]]
    """
    sentences = []
    sentence = []
...
def update_tag_scheme(sentences, tag_scheme):
    ...
    input:
        sentence: export from load_sentence()
        tag_scheme: 'iob' or 'iobes' (two different tagging rule)
...
def augment_with_pretrained(dictionary, ext_emb_path, words):
    ...
    Input:
        - dictionary: key: word, value: word vector
        - ext_emb_path: external pretrained word vector dataset path
        - words: add vocabulary given by words
    """
